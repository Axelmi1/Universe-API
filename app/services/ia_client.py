...
"""
            ...
            if force_json:
                ...
                logger.info(f"LLM response received (attempt {attempt + 1}, tokens: ~{len(content)//4})")
                
                # Parse JSON directly - no regex needed with JSON mode
                try:
                    return json.loads(content)
                except json.JSONDecodeError as e:
                    logger.warning(f"JSON parsing failed despite JSON mode (attempt {attempt + 1}): {str(e)}")
                    raise e
