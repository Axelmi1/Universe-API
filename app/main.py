...
async def log_requests(request: Request, call_next):
    ...
    logger.info(f"⏱️ Request processed in {process_time:.3f}s - Status: {response.status_code}")
...
fitness_metadata_router = APIRouter(prefix="/api/v1/metadata/fitness", tags=["Fitness Metadata"])
nutrition_metadata_router = APIRouter(prefix="/api/v1/metadata/nutrition", tags=["Nutrition Metadata"])  
...
async def public_get_fitness_levels():
    return {
        "fitness_levels": [
            {"id": "beginner", "name": "Beginner", "description": "New to exercise or returning after a long break"},
            {"id": "intermediate", "name": "Intermediate", "description": "Regular exercise for 3-6 months"},
            {"id": "advanced", "name": "Advanced", "description": "Consistent training for 1+ years"},
            {"id": "expert", "name": "Expert", "description": "Advanced athlete or trainer level"}
        ]
    }
...
async def public_get_equipment_options():
    return {
        "equipment_options": [
            {"id": "bodyweight", "name": "Bodyweight Only", "description": "No equipment needed"},
            {"id": "home_basic", "name": "Home Basic", "description": "Dumbbells, resistance bands"},
            {"id": "home_gym", "name": "Home Gym", "description": "Full home gym setup"},
...
async def public_get_fitness_goals():
    return {
        "goals": [
            {"id": "weight_loss", "name": "Weight Loss", "description": "Burn fat and lose weight"},
            {"id": "muscle_gain", "name": "Muscle Gain", "description": "Build muscle mass"},
            {"id": "strength", "name": "Strength", "description": "Increase overall strength"},
            {"id": "endurance", "name": "Endurance", "description": "Improve cardiovascular fitness"},
            {"id": "general_fitness", "name": "General Fitness", "description": "Overall health and wellness"},
            {"id": "athletic_performance", "name": "Athletic Performance", "description": "Sport-specific performance"}
        ]
    }
...
async def public_get_experience_levels():
    return {
        "fitness_levels": [
            {"id": "beginner", "name": "Beginner", "description": "New to the domain, needs foundational guidance"},
            {"id": "intermediate", "name": "Intermediate", "description": "Some experience, ready for structured approaches"},
            {"id": "advanced", "name": "Advanced", "description": "Significant experience, seeking optimization"},
            {"id": "expert", "name": "Expert", "description": "Extensive knowledge, interested in cutting-edge techniques"}
        ]
    }
...
async def public_get_challenges():
    return {
        "challenges": [
            {"id": "lack_of_motivation", "name": "Lack of Motivation", "description": "Struggling to stay motivated"},
            {"id": "time_constraints", "name": "Time Constraints", "description": "Limited time for health activities"},
            {"id": "budget_limitations", "name": "Budget Limitations", "description": "Financial constraints"},
            {"id": "lack_of_knowledge", "name": "Lack of Knowledge", "description": "Insufficient understanding"},
            {"id": "plateau", "name": "Plateau", "description": "Progress has stalled"},
            {"id": "lack_of_energy", "name": "Lack of Energy", "description": "Feeling tired or fatigued"},
            {"id": "social_support", "name": "Social Support", "description": "Lack of support from others"},
            {"id": "consistency", "name": "Consistency", "description": "Difficulty maintaining routine"}
        ]
    }
...
async def health_check():
    ...
    try:
        ...
        health_status = {
            ...
            "checks": {
                "api": "ok",
                "environment": "ok" if os.getenv("OPENAI_API_KEY") else "missing_config",
                "authentication": "ok" if os.getenv("MASTER_API_KEY") or not os.getenv("PRODUCTION") else "missing_config"
            }
        }
