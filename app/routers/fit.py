...
class WorkoutRequest(BaseModel):
    ...
    secondary_goals: List[FitnessGoalEnum] = Field(
        default=[], 
        max_items=2, 
        description="Secondary goals (max 2)"
    )
    ...
    session_duration: int = Field(
        ...
        ge=15,
        le=180,
        description="Desired session duration (minutes)"
    )
    ...
    preferred_workout_time: Optional[Literal["morning", "afternoon", "evening"]] = Field(
        default=None,
...
class Exercise(BaseModel):
    ...
    modifications: Optional[Dict[str, str]] = Field(
        default=None, 
        description="Modifications for different levels"
    )
...
def build_professional_prompt(req: WorkoutRequest) -> str:
    ...
    secondary_goals_str = ', '.join(g.value for g in req.secondary_goals) if req.secondary_goals else "none"
    ...
    **Features:**
    ...
    try:
        ...
        if os.getenv("PRODUCTION"):
            ...
            logger.debug(f"Generating workout for user: {request.age}yo {request.gender.value}, goal: {request.primary_goal.value}")
        
        prompt = build_professional_prompt(request)
        response = ask_llm(
            prompt, 
            max_tokens=1600  # Increased for complete workout structure
        )
        ...
        if not all(key in response for key in ['warmup', 'main_workout', 'cooldown']):
            raise HTTPException(
                status_code=500,
                detail="Incomplete workout structure received from AI"
            )
