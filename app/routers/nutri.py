...
class NutritionRequest(BaseModel):
    ...
    weight: float = Field(..., gt=20, lt=300, description="Current weight in kg")
    height: int = Field(..., ge=100, le=250, description="Height in cm")
    target_weight: Optional[float] = Field(
        None, gt=20, lt=300, description="Target weight in kg (if applicable)"
    )
    ...
    timeline_weeks: int = Field(
        default=12, ge=1, le=52, description="Timeline to achieve goal (weeks)"
    )

    # Dietary preferences
    dietary_restrictions: List[DietaryRestrictionsEnum] = Field(
        default=[], max_items=3, description="Dietary restrictions or preferences"
    )
    allergies: List[str] = Field(
        default=[], max_items=10, description="Food allergies or intolerances"
    )
    disliked_foods: List[str] = Field(
        default=[], max_items=15, description="Foods to avoid"
    )
    preferred_foods: List[str] = Field(
        default=[], max_items=15, description="Preferred foods"
    )
    ...
    meals_per_day: int = Field(
        default=3, ge=2, le=6, description="Preferred number of meals per day"
    )
    budget_per_week: Optional[float] = Field(
        None, gt=0, description="Weekly food budget (optional)"
    )

    # Health considerations
    health_conditions: List[str] = Field(
        default=[], max_items=5, description="Health conditions affecting nutrition"
    )
    medications: List[str] = Field(
        default=[], max_items=5, description="Medications that may affect nutrition"
    )
...
class Micronutrients(BaseModel):
    vitamin_d_mcg: float = Field(
        ..., description="Daily vitamin D requirement in micrograms"
    )
    vitamin_b12_mcg: float = Field(
        ..., description="Daily vitamin B12 requirement in micrograms"
    )
    iron_mg: float = Field(..., description="Daily iron requirement in milligrams")
    calcium_mg: float = Field(
        ..., description="Daily calcium requirement in milligrams"
    )
...
class HydrationGuidelines(BaseModel):
    daily_water_liters: float = Field(
        ..., description="Recommended daily water intake in liters"
    )
    electrolyte_needs: ElectrolyteNeeds = Field(
        ..., description="Electrolyte requirements"
    )
    timing_recommendations: List[str] = Field(
        ..., description="When to drink water throughout the day"
    )
...
class NutritionResponse(BaseModel):
    ...
    weekly_meal_prep: Dict[str, List[str]] = Field(
        ..., description="Meal preparation schedule"
    )
    shopping_list: List[str] = Field(..., description="Ingredient shopping list")
    recommended_supplements: List[NutritionalSupplement] = Field(
        ..., description="Supplement recommendations"
    )
    key_micronutrients: Micronutrients = Field(
        ..., description="Important micronutrient targets"
    )
    nutrition_education: List[str] = Field(..., description="Educational tips")
    hydration_guidelines: HydrationGuidelines = Field(
...
def build_professional_nutrition_prompt(request: NutritionRequest) -> str:
    ...
    # Budget context
    budget_text = (
        f"Budget: {request.budget_per_week}€/week. " if request.budget_per_week else ""
    )
...
async def generate_nutrition_plan(
    request: NutritionRequest = Body(
        ..., description="Personalized nutrition parameters"
    )
):
    """
    ...
    **Features:**
    ...
    try:
        ...
        if os.getenv("PRODUCTION"):
            logger.info(
                f"Generating nutrition plan for goal: {request.nutrition_goal.value}"
            )
        else:
            logger.debug(
                f"Generating nutrition plan for user: {request.age}yo {request.gender.value}, goal: {request.nutrition_goal.value}"
            )

        prompt = build_professional_nutrition_prompt(request)
        response = ask_llm(
            prompt, max_tokens=1200  # Increased for complete nutrition plans
        )

        # Additional response validation
        if not all(
            key in response for key in ["daily_calories", "daily_macros", "meals"]
        ):
            raise HTTPException(
                status_code=500, detail="Incomplete nutrition plan received from AI"
            )
