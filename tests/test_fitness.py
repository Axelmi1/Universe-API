...
class TestFitnessGeneration:
    ...
    @pytest.mark.asyncio
    async def test_workout_generation_success(self, async_client, openai_mock_fitness, valid_headers):
    ...
    @pytest.mark.asyncio
    async def test_workout_generation_validation_error_age(self, async_client, valid_headers):
    ...
    @pytest.mark.asyncio
    async def test_workout_generation_validation_error_sessions(self, async_client, valid_headers):
    ...
    @pytest.mark.asyncio
    async def test_workout_generation_with_challenges(self, async_client, openai_mock_fitness, valid_headers):
    ...
    @pytest.mark.asyncio
    async def test_workout_generation_advanced_user(self, async_client, openai_mock_fitness, valid_headers):
    ...
    @pytest.mark.asyncio
    async def test_workout_structure_validation(self, async_client, openai_mock_fitness, valid_headers):
        ...
        for phase_name in ["warmup", "main_workout", "cooldown"]:
            ...
            assert len(phase["exercises"]) > 0, f"{phase_name} should have at least one exercise"
            
            for exercise in phase["exercises"]:
                exercise_fields = ["name", "muscle_groups", "sets", "reps", "rest_time", "intensity", "technique_tips"]
                for field in exercise_fields:
                    assert field in exercise, f"Missing field {field} in exercise"

    @pytest.mark.asyncio
    async def test_workout_generation_with_health_conditions(self, async_client, openai_mock_fitness, valid_headers):
